...
void handle_Ecrire(jeton mot) {
    ...
    if (mot == NULL || strcmp(mot->data, "Ecrire") != 0) {
        erreur("Ecrire manquant", (mot != NULL) ? &mot->num_ligne : NULL);
        return;
    }
    ...
    // Check for "("
    if (temp != NULL && strcmp(temp->data, "(") == 0) {
        temp = temp->suivant;
        ...
        if (temp != NULL && strcmp(temp->data, "<<") == 0) {
            ...
            // Save the current line number for later comparison
            int ligne = temp->num_ligne;

            // Ignore everything between "<<" and ">>" while staying on the same line
            while (temp != NULL && strcmp(temp->data, ">>") != 0 && temp->num_ligne == ligne) {
                temp = temp->suivant;
            }
            ...
            if (temp != NULL && strcmp(temp->data, ">>") == 0) {
                ...
                if (temp != NULL && strcmp(temp->data, ")") == 0) {
                    ...
                    if (temp != NULL && strcmp(temp->data, ";") == 0) {
                        ...
                        erreur("Point-virgule manquant", (temp != NULL) ? &ligne : NULL);
                    }
                } else {
                    erreur("Parenthèse fermante manquante", (temp != NULL) ? &ligne : NULL);
                }
            } else {
                erreur("Guillemets angulaires fermants manquants", (temp != NULL) ? &ligne : NULL);
            }
        } else {
            erreur("Guillemets angulaires ouvrants manquants", (temp != NULL) ? &temp->num_ligne : NULL);
        }
    } else {
        erreur("Parenthèse ouvrante manquante", (temp != NULL) ? &temp->num_ligne : NULL);
    }
}
...
void handle_Lire(jeton mot) {
    jeton temp = mot;
    if (mot == NULL || strcmp(mot->data, "Lire") != 0) {
        erreur("Lire manquant", (mot != NULL) ? &mot->num_ligne : NULL);
        return;
    }
    ...
    if (temp != NULL && strcmp(temp->data, "(") == 0) {
        ...
        if (temp != NULL && strcmp(temp->data, ")") == 0) {
            ...
            erreur("Parenthese fermante manquante", (temp != NULL) ? &ligne : NULL);
        }
    } else {
        erreur("Parenthese ouvrante manquante", (temp != NULL) ? &temp->num_ligne : NULL);
    }
}
...
void handle_Debut(jeton mot) {
    jeton temp = mot;
    if (mot == NULL || strcmp(mot->data, "Debut") != 0) {
        erreur("Debut manquante", (mot != NULL) ? &temp->num_ligne : NULL);
        return;
    }

    temp = mot->suivant;

    // Process until "Fin" keyword is encountered or end of tokens
    while (temp != NULL && strcmp(temp->data, "Fin") != 0) {
        temp = temp->suivant;
    }

    // Check if "Fin" keyword is found
    if (temp != NULL && strcmp(temp->data, "Fin") == 0) {
        temp = temp->suivant;
        return;
        
    } else {
        erreur("Fin manquante a la fin", NULL);
...
void handle_Declaration(jeton mot) {
    ...
    // Check for "Variable" keyword
    if (temp != NULL && strcmp(temp->data, "Variable") == 0) {
        temp = temp->suivant;

        // Save the current line number for later comparison
        int ligne = temp->num_ligne;

        // Ignore the variable name while staying on the same line
        while (temp != NULL && strcmp(temp->data, ":") != 0 && temp->num_ligne == ligne) {
            temp = temp->suivant;
        }
        ...
        if (temp != NULL && strcmp(temp->data, ":") == 0) {
            ...
            if (!strcmp(temp->data, "entier") || !strcmp(temp->data, "reel") || !strcmp(temp->data, "char")) {
                temp = temp->suivant;

                // Check for ";" after the declaration
                if (temp != NULL && strcmp(temp->data, ";") == 0) {
                    temp = temp->suivant;
            ...
            erreur("Deux-points manquants", (temp != NULL) ? &ligne : NULL);
        }
    } else {
...
void handle_Si(jeton mot) {
    ...
    if (temp != NULL && strcmp(temp->data, "Si") == 0) {
        temp = temp->suivant;

        // Ignore everything until a valid condition is found while staying on the same line
        if (temp != NULL && !(strcmp(temp->data, "(") == 0 ))
        {
            temp = temp->suivant;
            if(temp != NULL && estChar(temp->data))
            {
                temp = temp->suivant;
                if (temp != NULL && (strcmp(temp->data, "<") == 0 || strcmp(temp->data, ">") == 0 || strcmp(temp->data, "<=") == 0 || strcmp(temp->data, ">=") == 0 || strcmp(temp->data, "!=") == 0 || strcmp(temp->data, "==") == 0 || strcmp(temp->data, ">>") == 0)) {
                    temp = temp->suivant;
                    int ligne2 = temp->num_ligne;
                    
                    // Check for ")"
                    // Ignore the cdt while staying on the same line
                    while (temp != NULL && strcmp(temp->data, ")") != 0 && temp->num_ligne == ligne2) {
                        temp = temp->suivant;
                    }
                    ...
                    if (temp != NULL && strcmp(temp->data, ")") == 0) {
                        ...
                        // Check for "alors"
                        if (temp != NULL && strcmp(temp->data, "alors") == 0) {
                            temp = temp->suivant;

                            // Ignore everything until "Sinon" is found
                            while (temp != NULL && strcmp(temp->data, "Sinon") != 0) {
                                temp = temp->suivant;
                            }

                            // Check for "Sinon"
                            if (temp != NULL && strcmp(temp->data, "Sinon") == 0) {
                                temp = temp->suivant;

                            } else {
                                erreur("Sinon manquant pour Si", (temp != NULL) ? &ligne2 : NULL);
                            }
                        } else {
                            erreur("alors manquant pour Si", (temp != NULL) ? &ligne2 : NULL);
                        }
                    } else {
                        erreur("parenthese fermante manquante pour Si", (temp != NULL) ? &ligne2 : NULL);
                    }
                } else {
                    erreur("Verifiez votre condition pour Si", (temp != NULL) ? &temp->num_ligne : NULL);
                }
            }else {
                 erreur("Verifiez votre Variable", (temp != NULL) ? &temp->num_ligne : NULL);
            }
        } else {
            erreur("parenthese ouvrante manquante", (temp != NULL) ? &temp->num_ligne : NULL);
        }
